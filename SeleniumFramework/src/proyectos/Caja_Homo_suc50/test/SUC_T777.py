...
from SeleniumFramework.constants.excel_constants import USUARIO, LEGAJO, TERMINAL, SUCURSAL,CLAVE,\
              ...
              DOCUMENTO1,DOCUMENTO2,DOCUMENTO3,DOCUMENTO4,DOCUMENTO5,USER_AUTO,PASSWORD_AUTO,TERMINAL_AUTO,\
...
@allure.testcase(u"ICaja - SUC-T777 - Depósito a Terceros - Superior a $ 280,000 - Por CBU")
@allure.title(u'SUC-T777 - Depósito en efectivo de 3° Superior a $ 280,000')
...
class SUC_T777(unittest.TestCase, stICajaInicio, stICaja,stICajaTickets,stAutorizInicio):
    def setUp(self):
        inicioICaja(self)
        
    def test_SUC_T777(self):
        try:
            self.usuario = get_user_caja_homo_suc_50(self._testMethodName)
            self.getDatos()
            self.login() 
            self.seleccionarDepositoTerceros()
            self.seleccionarTiposDeDocumentos(self.tipoDoc3)
            self.ingresarDatosDeCuentas("nroCbu",self.CBU)
            self.seleccionarSiguiente()
            self.ingresarImporteDepo3ros(self.importe)
            self.seleccionarSiguiente2()
            self.validarDatosCuenta(self.cuenta3)
            self.validarNombreDelTitular(self.nombreCliente3)
            ...
            self.seleccionarSiguiente3ros()
            self.validarDetalleDeOperacion(self.cuenta3,"pesos",self.importe)
