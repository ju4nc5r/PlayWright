...
from SeleniumFramework.constants.excel_constants import USUARIO, LEGAJO, TERMINAL, SUCURSAL,CLAVE,\
              CUENTA_VALIDAR1, NOMBRE_APELLIDO1, CUENTA1,\
              MONEDA,IMPORTE, MSJ_ESPERADO,SUCURSAL_DEST,IMPORTE1,IMPORTE2,\
              IMPORTE_VALIDAR_1,OPERACION1,TITULO_ESPERADO,NUMERO_TAS,OPERACION_A_VALIDAR,SUC_JOB,USER_AUTO,PASSWORD_AUTO,TERMINAL_AUTO     

@allure.feature(u'TAS')
@allure.story(u'TAS Liberaciones')
@allure.testcase(u"ICaja - SUC-T866 - TAS Liberaciones Deposito Cheque Propios")
@allure.title(u'SUC-T866 - TAS Liberaciones Deposito Cheque Propios')
...
14.-Ingresar los datos del cheque desde la aplicación "CRM"</br> 
15.-Hacer clic en cualquier espacio de la pantalla</br> 
...
19.-Visualizar la impresión del ticket
...
class SUC_T866(unittest.TestCase, stICajaInicio, stICaja,stBusquedaClientes,stAutorizInicio,stICajaTickets):
    def setUp(self):
        inicioICaja(self)
        
    def test_SUC_T866(self):
        try:
            ...
            self.ingresar_cuenta4("00003071009")
            self.ingresar_datos_de_chequera_2()  
            self.seleccionarFindeCarga()
            self.validar_sucursal(self.sucursal_job)
    ...
    def getDatos(self):
        self.sucursal_job = self.usuario.get(SUC_JOB)
