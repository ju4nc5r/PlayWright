# -*- coding: utf-8 -*-
from SeleniumFramework.src.proyectos.Caja_Homo_suc50.pageLoc.plAdminConsulta import plAdminConsulta
...
#                 Se hace foco en el botón Imprimir (totales por sucursal) y se da enter y escape para imprimir y cerrar dialogo impresión 
    ...
    def visuaizarItemVer(self):
        to = 10
        accion = "Vizualizar item VER"
        msgOk, msgFail = get_msg(accion)
        with self.step(accion):
            xpath = plAdminConsulta.btn_preview
            self.go_to_xpath(xpath)
            if self.visibility_element(xpath, to):
    ...
    def seleccionarImprimirTicket(self):
        to = 10
        accion = 'Seleccionar Imprimir Ticket'
        msgOk, msgFail = get_msg(accion)
        with self.step(accion):
            self.driver.execute_script("window.scrollTo(0,700);") #Se hace scrool porque el goto_xpath no funciona en este caso
            time.sleep(1)
            xpath = plAdminConsulta.btn_imprmirTicket
            if self.visibility_element(xpath, to):
                self.go_to_xpath(xpath)                
                self.highlight(xpath, accion)
                self.selectElement(xpath, msgOk, msgFail, to)
                self.driver.switch_to.window(self.driver.window_handles[1])
                xpath = plAdminConsulta.ticket
                self.highlight(xpath, "ticket")  
                self.driver.switch_to.window(self.driver.window_handles[0])
            else:
                self.fail_msg(msgFail) 
    ...
    def seleccionarBuscar(self):
        to = 30
        accion = u'Hacer clic en el Boton BUSCAR'
        msgOk = accion
        msgFail = u'No se pudo '+ msgOk.lower()
        with self.step(accion):
            xpath = plAdminConsulta.btn_buscar
            if self.visibility_element(xpath, to):
                self.highlight(xpath, accion)
                self.jsClick(xpath)
            else:
    ...
    def seleccionarSalir(self):
        ...
        msgFail = u'No se pudo '+ msgOk.lower()
        with self.step(accion):
            xpath = plAdminConsulta.btn_salir2
            if self.visibility_element(xpath, to):
                self.highlight(xpath, accion)
                self.jsClick(xpath)
            else:
