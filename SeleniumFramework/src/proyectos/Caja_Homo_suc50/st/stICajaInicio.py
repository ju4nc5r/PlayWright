# -*- coding: utf-8 -*-
from SeleniumFramework.src.proyectos.Caja_Homo_suc50.pageLoc.pl_ICajaInicio import pl_ICajaInicio
...
class stICajaInicio(abrirNavegador):
    ...
    def apertura_forzada(self):
        posiciones = ["1","2","3","4"]
        for posicion in posiciones:
            self.cargar_saldos_aperador(posicion)
    ...
    def seleccionar_siguiente(self):
        to = 10
        accion = 'Seleccionar siguiente'
        msgOk, msgFail = get_msg(accion)
        msgFail = "nose pudo",msgOk
        xpath=pl_ICajaInicio.button_siguiente
    ...
    def verificar_titulo_apertura_forzada(self):
        accion = 'Validar titulo apertura forzada'
        msgOk = accion
        with self.step(accion):         
            xpath = pl_ICajaInicio.tle_apertura_forzada
            to = 10   
            if self.visibility_element(xpath, to): 
    ...
    def cargar_saldos_aperador(self,posicion):
        to = 10
        accion = 'cargar saldos aperador {}'.format(posicion)
        msgOk, msgFail = get_msg(accion)
        msgFail = "nose pudo",msgOk
        xpath=pl_ICajaInicio.checkbox_carga.format(posicion)
        with self.step(accion):
            if self.visibility_element(xpath, to):
                self.selectElement(xpath, msgOk, msgFail, to)
            else:
...
#         to = 60
    ...
    def validar_usuario(self, user):
        to = 10
        accion = u'Validar nombre del usuario {}'.format(user)
        msgOk, msgFail = get_msg(accion)
        with self.step(accion):
            xpath = pl_ICajaInicio.txt_usuario.format(user)
            if self.visibility_element(xpath):
                texto_motrado = self.driver.find_element(by=By.XPATH, value=xpath).get_property('innerText')
                if user in texto_motrado:
                    self.highlight(xpath, msgOk)
            else:
                self.fail_msg(msgFail)
...
# Cerrar Sesión
    ...
    def CerrarDatosOperador(self):
        to = 2
        accion = u'Hacer clic en el recuadro Hola, UUIC10021 para que desaparezca los datos del operador'
        msgOk, msgFail = get_msg(accion)
        with self.step(accion):     
            import os
            self.wait(to)
...
#     def CerrarDatosOperador(self):
#         to = 10
#         accion = u'Hacer clic en el recuadro Hola, UUIC10021para que desaparezca los datos del operador'
#         msgOk, msgFail = get_msg(accion)
#         with self.step(accion):
