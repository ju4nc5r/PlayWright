...
from SeleniumFramework.constants.excel_constants import USUARIO, LEGAJO, TERMINAL, CLAVE,\
              ...
              MONEDA, IMPORTE, MSJ_ESPERADO,USER_AUTO,PASSWORD_AUTO,TERMINAL_AUTO
             

@allure.feature(u'Operaciones de Caja')
@allure.story(u'Reverso de Transacción')
@allure.testcase(u"ICaja SUC-T833 - Reverso de Transacción - Extracción en Efectivo")
@allure.title(u'SUC-T833 - Reverso de Transacción - Extracción en Efectivo')
...
16.-Ingresar importe "5000,00"</br>
...
class SUC_T833(unittest.TestCase, stICajaInicio, stICaja, stICajaTickets,stAutorizInicio):
    def setUp(self):
        inicioICaja(self)
        
    def test_SUC_T833(self):
        try:
            ...
            self.visualizarDatosOperacion(self.importe,self.cuenta,self.tipoMoneda)
            self.seleccionarConfirmar()   
            self.verificarMensajeExitoExtracciones(self.mensaje)
            self.verificarNumeroDeTransaccion()
