...
@allure.story(u'Compra Moneda Extranjera')
@allure.testcase(u"SUC-T878 - Compra Moneda Extranjera")
@allure.title(u'SUC-T878 - Compra Moneda Extranjera')
...
5.-Hacer clic en la trassacicion 119 Compra Moneda Extranjera</br>
6.-Visualizar en pantalla el Recuadro de los Datos Iniciales</br>
7.-Seleccionar tipo de docuento "DNI" e ingresar Numero de DNI "92635353"</br>
8.-Seleccionar Debe Presentar "CUIL" e ingresar Numero de CUIL "20926353536"</br>
9.-Seleccionar Moneda "Dolares"</br>
10.-Hacer clic en el Botón "SIGUIENTE"</br>
11.-Ingresar Importe a comprar "1,00"</br>
12.-Hacer clic en el boton "COTIZAR"</br>
13.-Visualizar en la cotizacion e importe pesos compra</br>
...
class SUC_T878(unittest.TestCase, stICajaInicio, stICaja , stICajaTickets):
    def setUp(self):
        inicioICaja(self)
        
    def test_SUC_T878(self):
        try:
            self.usuario = get_user_caja_inte(self._testMethodName)
            self.getDatos()
            self.login()
            self.seleccionar_compra_moneda_extranjera()
            self.seleccionar_tipo_documento()
            self.seleccionar_item_DNI()
            self.ingresar_numero_de_DNI("92635353")
            self.seleccionar_debe_precentar()
            self.seleccionar_item_CUIL()
            self.ingresar_numero_CUIL("20926353536")
