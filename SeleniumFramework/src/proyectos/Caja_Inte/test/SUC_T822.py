...
@allure.testcase(u"SUC-T822 - Envió de Remesas Interna CAJA - CAF Antes del Cierre de Caja (UIC10021)")
@allure.title(u'SUC-T822 - Envió de Remesas Interna CAJA - CAF Antes del Cierre de Caja (UIC10021)')
...
class SUC_T822(unittest.TestCase, stICajaInicio, stICaja, stICajaTickets):
    def setUp(self):
        inicioICaja(self)
        
    def test_SUC_T822(self):
        try:
            ...
            self.verificarSaldoDeRemesasDeCheques()
            if self.saldoCheques !="0,00":
                print(self.saldoCheques)              
                self.envioDeRemesasDeChequesAntesDelCierre()                
            ...
            for moneda,posicion in zip(self.monedas,self.posiciones1):  
                ...
                self.validar_origen("Caja")
                self.validar_destino("Caf")
                self.validar_moneda(moneda)
                self.validarImporteOpn(self.monedaSaldo)
