...
from SeleniumFramework.constants.excel_constants import USUARIO, LEGAJO, TERMINAL, SUCURSAL,CLAVE,\
              MONEDA, MSJ_ESPERADO, NRO_CONTROL, SUCURSAL_DEST,SUCURSAL_ORIG,\
...
@allure.testcase(u"ICaja - SUC-T850 - Cobros Varios")
@allure.title(u'SUC-T850 - Cobros Varios - Moneda Pesos')
@allure.link(u'https://jira.sis.ad.bia.itau/secure/Tests.jspa#/testCase/35800')
...
7.-Seleccionar el ítem "49 Otros Créditos"</br>
8.-Hacer clic en seleccionar Moneda "Pesos"</br>
...
class SUC_T850(unittest.TestCase, stICajaInicio, stICaja,stAutorizInicio, stICajaTickets):
    def setUp(self):
        inicioICaja(self)
        
    def test_SUC_T850(self):
        try:
            ...
            self.wait(3)
            self.seleccionarTipoDeOperacion(self.Operacion1)
            self.seleccionarComboBox()          
            self.seleccionarMoneda(self.moneda1)        
            self.ingresarImporte2(self.importe1)
            self.seleccionarBtnSiguiente() 
            self.validarTipoDeOperacion(self.operacionAValidar1)
            self.validarMonedaOpn(self.moneda1)
            self.validarImporteOpn(self.importeValidar1)  
