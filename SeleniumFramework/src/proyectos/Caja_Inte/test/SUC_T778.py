...
@allure.testcase(u"ICaja - SUC-T778 - Depósito en efectivo de 3° Superior a $ 280,000 - Por Alias")
@allure.title(u'SUC-T778 - Depósito en efectivo de 3° Superior a $ 280,000')
...
6-Seleccionar Alias</br>
7-Seleccionar el recuadro Alias e ingresar Alias: BABY.TESTING.CC</br>
...
class SUC_T778(unittest.TestCase, stICajaInicio, stICaja,stICajaTickets,stAutorizInicio):
    def setUp(self):
        inicioICaja(self)
        
    def test_SUC_T778(self):
        try:
            self.usuario = get_user_caja_inte(self._testMethodName)
            self.getDatos()
            self.login() 
            self.seleccionarDepositoTerceros()
            self.seleccionarTiposDeDocumentos(self.tipoDoc5)
            self.ingresarDatosDeCuentas("alias",self.Alias)
            self.seleccionarSiguiente()      
            self.ingresarImporteDepo3ros(self.importe)
            self.seleccionarSiguiente2()
            self.validarDatosCuenta(self.cuenta5)
            self.validarNombreDelTitular(self.nombreCliente5)
            ...
            self.seleccionarSiguiente3ros()
            self.validarDetalleDeOperacion(self.cuenta5,"pesos",self.importe)
