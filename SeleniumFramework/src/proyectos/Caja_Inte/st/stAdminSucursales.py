# -*- coding: utf-8 -*-
from SeleniumFramework.src.proyectos.Caja_Inte.pageLoc.plAdminSucursales import plAdminSucursales
...
class stAdminSucursales(abrirNavegador):
    ...
    def resaltar_msj_cierre_forzado(self):
        ...
        with self.step(accion):
            ...
            if self.visibility_element(xpath, to):
                self.highlight(xpath, accion)
                self.skip_msg("Se procederá a la apertura forzada de la sucursal")
            else:
                pass               
