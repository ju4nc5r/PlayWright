...
@allure.description(
    u"""En la pantalla de Resultado, Usuario presiona bot�n "Continuar" </br>
    1- Ingresar con un usuario que posea tarjetas de cr�dito </br>
    2- Ir al men� Pagos-->Tarjeta de cr�dito y clickear
    <b>Pago de tarjeta de cr�dito</b> </br>
...
class HB_T185(unittest.TestCase, stPagosTC):
    def setUp(self):
        inicio_test(self)

    def test_HB_T185(self):
        try:
            self.usuario = get_user_hb(self._testMethodName)
            self.get_datos()
            self.login()
            self.seleccionarMenuPagos()
            self.seleccionarAnularPago()
            self.seleccionarTarjeta(self.tarjeta)
