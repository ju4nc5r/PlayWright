...
@allure.testcase(u"HB-T108 -0035- Cambio de clave - clave nueva y repetición no coinciden")
@allure.title(u"HB-T108 -0035- Cambio de clave - clave nueva y repetición no coinciden")
@allure.severity(allure.severity_level.NORMAL)
@allure.link(u"https://jira.sis.ad.bia.itau/secure/Tests.jspa#/testCase/HB-T108")
@allure.description(
    u"""Realizar cambio de clave </br>
    Condición de prueba: Usuario ingresa clave actual correcta y clave
    nueva y repetición no coinciden </br>
    pasos: </br>
    1-Hacer click en la rueda de configuración </br>
    2-Hacer click en el cambio de clave </br>
    3-Ingresar clave actual correcta y clave nueva y repetición
    no coinciden </br>
...
class HB_T108 (unittest.TestCase, stLogin, stCambioClave):
    def setUp(self):
        inicio_test(self)

    def test_CDPF_T145(self):
        try:
            self.usuario = get_user_hb(self._testMethodName)
            self.getDatos()
            self.login()
            self.cambiarClave(True, False, True)
            self.finalizo = True
        except Exception:
