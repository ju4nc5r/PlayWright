...
class MsgBrowser():
    ...
    # WINDOWS SIZE
    MSG_WINDOW_SIZE = (
        "Get the window size. The size is %s." % __SIZE
    )
...
class MsgSearch():
    ...
    MSG_COMPARE_TEXT_ERROR = (
        "The text {0}, doesn't show in searched element {1}"
    )
    DSC_COMPARE_TEXT = "Expected value {0}, found value {1}"
    MSG_COMPARE_TEXT_FAIL = 'The text are different'
    # VERIFY
    MSG_VERIFY = "Verify: {}"
    ...
    MSG_ARRAY_SEARCH = 'Start array search'
...
class MsgSelect():
    ...
    MSG_SELECT_INDEX = (
        "From element {1}, select option by {0} index"
    )
    MSG_SELECT_VALUE = (
        "From element {1}, select option by {0} value"
    )
    ...
    MSG_SELECT_ELEMENT_ERROR = (
        "The searched element {}, isn't a select element"
    )
...
class MsgWebelement():
    ...
    MSG_ATTRIBUTE_ERROR = (
        "Can't get the attribute {0}, from the element {1}"
    )
...
# UPLOADFILE
MSG_UPLOAD = 'Starting the method to upload the file'
MSG_UPLOAD_DONE = 'The file {} was uploaded'
MSG_UPLOAD_FAIL = 'Invalid file {}'
