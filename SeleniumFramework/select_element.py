...
from SeleniumFramework.constants.spanish_messages import MsgSelect, MsgSearch
from SeleniumFramework.constants.constants import ARIA_OWNS, TAG_LI, SCROLL
...
class select(object):
    ...
    def select_option(
            self, xpath, msgOk, msgFail, to=5, option=None, arrow=None):
        """
        ...
        if type(option) is int:
            ...
            try:
                ...
                self.log.error(MsgSelect.MSG_SELECT_OPTION_ERROR.format(
                                                                    option))
                self.fail(MsgSelect.MSG_SELECT_OPTION_ERROR.format(option)) 
        if flag:
            try:
    ...
    def selectListByPartialText(self, xpath, find_text):
        ...
        if self.visibility_element(xpath, 10):
            ...
            self.skip_msg(MsgSearch.MSG_SEARCH_ELEM_ERROR.format(find_text))
            return False
        ...
        for option in options:
            if find_text in option.text and option.is_enabled():
                ...
                break
        if index is None:
            self.log.error(MsgSelect.MSG_SELECT_OPTION_ERROR.format(find_text))
            self.skip_msg(MsgSearch.MSG_SEARCH_ELEM_ERROR.format(find_text))
            return False
        select.select_by_index(index) 
        selected_option = select.options[index].text
        self.log.info(MsgSelect.MSG_SELECT_OPTION.format(
                                            selected_option, xpath))
        return True
